...
class BillService {
  ...
  /// Recalculate all member totals for a bill
  Future<void> recalculateBillMemberTotals(String billId) async {
    try {
      // 1. Get bill details for tax and service calculation
      final billResponse = await _supabase
          .from('bills')
          .select()
          .eq('id', billId)
      ...
      // 2. Get all items with their assignments
      ...
      for (final itemJson in items) {
        ...
        final assignments = itemJson['item_assignments'] as List;
      ...
      // 4. Apply tax and service charge
      final taxRate = bill.taxPercent / 100;
      final serviceRate = bill.servicePercent / 100;

      for (final entry in userTotals.entries) {
        final subtotal = entry.value;
        final tax = subtotal * taxRate;
        final service = subtotal * serviceRate;
        userTotals[entry.key] = subtotal + tax + service;
      }

      // 5. Update all bill_members with new totals
      for (final entry in userTotals.entries) {
        await _supabase
            .from('bill_members')
            .update({'final_total': entry.value})
            .eq('bill_id', billId)
            .eq('user_id', entry.key);
      }

      // 6. Set final_total to 0 for users not in userTotals (no assignments)
      final allMembersResponse = await _supabase
          .from('bill_members')
          .select('user_id')
          .eq('bill_id', billId);

      final allMemberIds = (allMembersResponse as List)
          .map((m) => m['user_id'] as String)
          .toList();

      for (final memberId in allMemberIds) {
        if (!userTotals.containsKey(memberId)) {
          await _supabase
              .from('bill_members')
              .update({'final_total': 0})
              .eq('bill_id', billId)
              .eq('user_id', memberId);
        }
      }
    } catch (e) {
      throw Exception('Failed to recalculate member totals: $e');
    }
  }
